...
import { Property, InputProperty } from "../../types/interfaces";

const accommodationRouter = express.Router();

accommodationRouter.get("/", async (req, res) => {
  const accommodation: Property[] = await AccommodationModel.find({});
  res.status(200).send({ accommodation });
});

accommodationRouter.get("/:id", async (req, res) => {
  try {
    const accommodation: Property[] = await AccommodationModel.findById(
      req.params.id
    );
    if (!accommodation) {
      res.status(404).send();
...
accommodationRouter.post("/", async (req, res) => {
  try {
    const { name, description, maxGuests, city }: InputProperty = req.body;
...
accommodationRouter.put("/:id", async (req, res, next) => {
  try {
    ...
    const _accommodation = await AccommodationModel.findByIdAndUpdate(
      req.params.id,
      req.body,
...
accommodationRouter.delete("/:id", async (req, res, next) => {
  try {
    ...
    const _accommodation = await AccommodationModel.findByIdAndDelete(
      req.params.id
    );
    if (_accommodation) {
      res.status(204).send();
